...
if (NOT AD_SKIP_FETCHCONTENT)
    ...
    message(STATUS "AD_SKIP_FETCHCONTENT=ON: creating local INTERFACE stubs for doom_st, refl-cpp, doom_meta(expected)")
    if (NOT TARGET doom_st)
        add_library(doom_st INTERFACE)
    endif()
    if (NOT TARGET refl-cpp)
        add_library(refl-cpp INTERFACE)
        # If local source exists, add include dir
        if (EXISTS "${CMAKE_SOURCE_DIR}/vendor/refl-cpp")
            target_include_directories(refl-cpp INTERFACE ${CMAKE_SOURCE_DIR}/vendor/refl-cpp)
        endif()
    endif()
    if (NOT TARGET expected)
        add_library(expected INTERFACE)
        if (EXISTS "${CMAKE_SOURCE_DIR}/vendor/expected")
            target_include_directories(expected INTERFACE ${CMAKE_SOURCE_DIR}/vendor/expected)
        endif()
    endif()
    # Provide a doom::meta alias target when skipping fetchcontent so vendor code can link
...
if (NOT AD_SKIP_FETCHCONTENT)
    ...
    # AD_SKIP_FETCHCONTENT: refl-cpp should already be defined as a stub earlier
    if (TARGET refl-cpp)
        add_library(antara::refl-cpp ALIAS refl-cpp)
    endif()
endif()
