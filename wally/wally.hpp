...
namespace detail {
template <class P>
inline auto get_p(const P &p, std::false_type, std::true_type) {
  return &p[0];
}
template <class P>
inline auto get_p(const P &p, std::true_type, std::false_type) {
  return p;
}
template <class P>
inline auto get_p(const P &p, std::false_type, std::false_type) {
  return p.get();
}
template <class P> inline auto get_p(const P &p) {
  return get_p(p, std::is_pointer<P>{}, std::is_array<P>{});
}
template <> inline auto get_p(const std::string &p) { return p.c_str(); }
template <> inline auto get_p(const std::nullptr_t &p) { return p; }
} // namespace detail

#define WALLYP(var) detail::get_p(var)

#define WALLYB(var) var.data(), var.size()

#define WALLYO(var) var.data(), var.size()

#define WALLY_FN_3(F, N)                                                       \
  inline int F(uint32_t i321) { return ::N(i321); }

#define WALLY_FN_333_BBBBBA(F, N)                                              \
  template <class I1, class I2, class I3, class I4, class I5, class O>         \
  inline int F(uint32_t i321, uint32_t i322, uint32_t i323, const I1 &i1,      \
               const I2 &i2, const I3 &i3, const I4 &i4, const I5 &i5,         \
               O **out) {                                                      \
    return ::N(i321, i322, i323, WALLYB(i1), WALLYB(i2), WALLYB(i3),           \
               WALLYB(i4), WALLYB(i5), out);                                   \
  }

#define WALLY_FN_33SS_A(F, N)                                                  \
  template <class O>                                                           \
  inline int F(uint32_t i321, uint32_t i322, size_t s1, size_t s2, O **out) {  \
    return ::N(i321, i322, s1, s2, out);                                       \
  }

#define WALLY_FN_3_A(F, N)                                                     \
  template <class O> inline int F(uint32_t i321, O **out) {                    \
    return ::N(i321, out);                                                     \
  }

#define WALLY_FN_6_B(F, N)                                                     \
  template <class O> inline int F(uint64_t i641, O &out) {                     \
    return ::N(i641, WALLYO(out));                                             \
  }

#define WALLY_FN_6B_A(F, N)                                                    \
  template <class I1, class O>                                                 \
  inline int F(uint64_t i641, const I1 &i1, O **out) {                         \
    return ::N(i641, WALLYB(i1), out);                                         \
  }

#define WALLY_FN_6BBBBBB_A(F, N)                                               \
  template <class I1, class I2, class I3, class I4, class I5, class I6,        \
            class O>                                                           \
  inline int F(uint64_t i641, const I1 &i1, const I2 &i2, const I3 &i3,        \
               const I4 &i4, const I5 &i5, const I6 &i6, O **out) {            \
    return ::N(i641, WALLYB(i1), WALLYB(i2), WALLYB(i3), WALLYB(i4),           \
               WALLYB(i5), WALLYB(i6), out);                                   \
  }

#define WALLY_FN_B(F, N)                                                       \
  template <class I> inline int F(const I &i1) { return ::N(WALLYB(i1)); }

#define WALLY_FN_B33BP_A(F, N)                                                 \
  template <class I1, class I2, class P1, class O>                             \
  inline int F(const I1 &i1, uint32_t i321, uint32_t i322, const I2 &i2,       \
               const P1 &p1, O **out) {                                        \
    return ::N(WALLYB(i1), i321, i322, WALLYB(i2), WALLYP(p1), out);           \
  }

#define WALLY_FN_B33_A(F, N)                                                   \
  template <class I1, class O>                                                 \
  inline int F(const I1 &i1, uint32_t i321, uint32_t i322, O **out) {          \
    return ::N(WALLYB(i1), i321, i322, out);                                   \
  }

#define WALLY_FN_B33_BS(F, N)                                                  \
  template <class I1, class O>                                                 \
  inline int F(const I1 &i1, uint32_t i321, uint32_t i322, O &out,             \
               size_t *written = 0) {                                          \
    size_t n;                                                                  \
    int ret =                                                                  \
        ::N(WALLYB(i1), i321, i322, WALLYO(out), written ? written : &n);      \
    return written || ret != WALLY_OK             ? ret                        \
           : n == static_cast<size_t>(out.size()) ? WALLY_OK                   \
                                                  : WALLY_EINVAL;              \
  }

#define WALLY_FN_B33_P(F, N)                                                   \
  template <class I1, class O>                                                 \
  inline int F(const I1 &i1, uint32_t i321, uint32_t i322, O *out) {           \
    return ::N(WALLYB(i1), i321, i322, out);                                   \
  }

#define WALLY_FN_B3_A(F, N)                                                    \
  template <class I1, class O>                                                 \
  inline int F(const I1 &i1, uint32_t i321, O **out) {                         \
    return ::N(WALLYB(i1), i321, out);                                         \
  }

#define WALLY_FN_B3_BS(F, N)                                                   \
  template <class I1, class O>                                                 \
  inline int F(const I1 &i1, uint32_t i321, O &out, size_t *written = 0) {     \
    size_t n;                                                                  \
...
#define WALLY_FN_B3_B(F, N)                                                    \
  template <class I1, class O>                                                 \
  inline int F(const I1 &i1, uint32_t i321, O &out) {                          \
    return ::N(WALLYB(i1), i321, WALLYO(out));                                 \
  }

#define WALLY_FN_B3B_B(F, N)                                                   \
  template <class I1, class I2, class O>                                       \
  inline int F(const I1 &i1, uint32_t i321, const I2 &i2, O &out) {            \
    return ::N(WALLYB(i1), i321, WALLYB(i2), WALLYO(out));                     \
  }

#define WALLY_FN_BB333_B(F, N)                                                 \
  template <class I1, class I2, class O>                                       \
  inline int F(const I1 &i1, const I2 &i2, uint32_t i321, uint32_t i322,       \
               uint32_t i323, O &out) {                                        \
    return ::N(WALLYB(i1), WALLYB(i2), i321, i322, i323, WALLYO(out));         \
  }

#define WALLY_FN_BB3_A(F, N)                                                   \
  template <class I1, class I2, class O>                                       \
  inline int F(const I1 &i1, const I2 &i2, uint32_t i321, O **out) {           \
    return ::N(WALLYB(i1), WALLYB(i2), i321, out);                             \
  }

#define WALLY_FN_BB3_B(F, N)                                                   \
  template <class I1, class I2, class O>                                       \
  inline int F(const I1 &i1, const I2 &i2, uint32_t i321, O &out) {            \
    return ::N(WALLYB(i1), WALLYB(i2), i321, WALLYO(out));                     \
  }

#define WALLY_FN_BB3_BS(F, N)                                                  \
  template <class I1, class I2, class O>                                       \
  inline int F(const I1 &i1, const I2 &i2, uint32_t i321, O &out,              \
               size_t *written = 0) {                                          \
    size_t n;                                                                  \
    int ret = ::N(WALLYB(i1), WALLYB(i2), i321, WALLYO(out),                   \
                  written ? written : &n);                                     \
    return written || ret != WALLY_OK             ? ret                        \
           : n == static_cast<size_t>(out.size()) ? WALLY_OK                   \
                                                  : WALLY_EINVAL;              \
...
#define WALLY_FN_BBB3_BS(F, N)                                                 \
  template <class I1, class I2, class I3, class O>                             \
  inline int F(const I1 &i1, const I2 &i2, const I3 &i3, uint32_t i321,        \
               O &out, size_t *written = 0) {                                  \
    size_t n;                                                                  \
    int ret = ::N(WALLYB(i1), WALLYB(i2), WALLYB(i3), i321, WALLYO(out),       \
                  written ? written : &n);                                     \
    return written || ret != WALLY_OK             ? ret                        \
           : n == static_cast<size_t>(out.size()) ? WALLY_OK                   \
                                                  : WALLY_EINVAL;              \
...
#define WALLY_FN_BB_B(F, N)                                                    \
  template <class I1, class I2, class O>                                       \
  inline int F(const I1 &i1, const I2 &i2, O &out) {                           \
    return ::N(WALLYB(i1), WALLYB(i2), WALLYO(out));                           \
  }

#define WALLY_FN_BP3_A(F, N)                                                   \
  template <class I1, class P1, class O>                                       \
  inline int F(const I1 &i1, const P1 &p1, uint32_t i321, O **out) {           \
    return ::N(WALLYB(i1), WALLYP(p1), i321, out);                             \
  }

#define WALLY_FN_BB_BS(F, N)                                                   \
  template <class I1, class I2, class O>                                       \
  inline int F(const I1 &i1, const I2 &i2, O &out, size_t *written = 0) {      \
    size_t n;                                                                  \
    int ret =                                                                  \
        ::N(WALLYB(i1), WALLYB(i2), WALLYO(out), written ? written : &n);      \
    return written || ret != WALLY_OK             ? ret                        \
           : n == static_cast<size_t>(out.size()) ? WALLY_OK                   \
                                                  : WALLY_EINVAL;              \
  }

#define WALLY_FN_B_A(F, N)                                                     \
  template <class I1, class O> inline int F(const I1 &i1, O **out) {           \
    return ::N(WALLYB(i1), out);                                               \
  }

#define WALLY_FN_B_B(F, N)                                                     \
  template <class I, class O> inline int F(const I &i1, O &out) {              \
    return ::N(WALLYB(i1), WALLYO(out));                                       \
  }

#define WALLY_FN_B_BS(F, N)                                                    \
  template <class I, class O>                                                  \
  inline int F(const I &i1, O &out, size_t *written = 0) {                     \
    size_t n;                                                                  \
...
#define WALLY_FN_B_P(F, N)                                                     \
  template <class I1, class O> inline int F(const I1 &i1, O *out) {            \
    return ::N(WALLYB(i1), out);                                               \
  }

#define WALLY_FN_B_S(F, N)                                                     \
  template <class I> inline int F(const I &i1, size_t *written) {              \
    return ::N(WALLYB(i1), written);                                           \
  }

#define WALLY_FN_BB33_B(F, N)                                                  \
  template <class I1, class I2, class O>                                       \
  inline int F(const I1 &i1, const I2 &i2, uint32_t i321, uint32_t i322,       \
               O &out) {                                                       \
...
#define WALLY_FN_P(F, N)                                                       \
  template <class P1> inline int F(const P1 &p1) { return ::N(WALLYP(p1)); }

#define WALLY_FN_P3(F, N)                                                      \
  template <class P1> inline int F(const P1 &p1, uint32_t i321) {              \
    return ::N(WALLYP(p1), i321);                                              \
  }

#define WALLY_FN_P33(F, N)                                                     \
  template <class P1>                                                          \
  inline int F(const P1 &p1, uint32_t i321, uint32_t i322) {                   \
    return ::N(WALLYP(p1), i321, i322);                                        \
  }

#define WALLY_FN_P33_P(F, N)                                                   \
  template <class P1, class O>                                                 \
  inline int F(const P1 &p1, uint32_t i321, uint32_t i322, O *out) {           \
    return ::N(WALLYP(p1), i321, i322, out);                                   \
  }

#define WALLY_FN_P3B(F, N)                                                     \
  template <class P1, class I1, class O>                                       \
  inline int F(const P1 &p1, uint32_t i321, const I1 &i1, O &out) {            \
    return ::N(WALLYP(p1), i321, WALLYB(i1), WALLYO(out));                     \
  }

#define WALLY_FN_P3B633_B(F, N)                                                \
  template <class P1, class I1, class O>                                       \
  inline int F(const P1 &p1, uint32_t i321, const I1 &i1, uint64_t i641,       \
               uint32_t i322, uint32_t i323, O &out) {                         \
...
#define WALLY_FN_P3BB36333_B(F, N)                                             \
  ...
  inline int F(const P1 &p1, uint32_t i321, const I1 &i1, const I2 &i2,        \
               uint32_t i322, uint64_t i641, uint32_t i323, uint32_t i324,     \
               uint32_t i325, O &out) {                                        \
    return ::N(WALLYP(p1), i321, WALLYB(i1), WALLYB(i2), i322, i641, i323,     \
               i324, i325, WALLYO(out));                                       \
  }

#define WALLY_FN_P3_A(F, N)                                                    \
  template <class P1, class O>                                                 \
  inline int F(const P1 &p1, uint32_t i321, O **out) {                         \
    return ::N(WALLYP(p1), i321, out);                                         \
  }

#define WALLY_FN_P3_B(F, N)                                                    \
  template <class P1, class O>                                                 \
  inline int F(const P1 &p1, uint32_t i321, O &out) {                          \
    return ::N(WALLYP(p1), i321, WALLYO(out));                                 \
  }

#define WALLY_FN_P3_BS(F, N)                                                   \
  template <class P1, class O>                                                 \
  inline int F(const P1 &p1, uint32_t i321, O &out, size_t *written = 0) {     \
    size_t n;                                                                  \
...
#define WALLY_FN_P3_S(F, N)                                                    \
  template <class P1>                                                          \
  inline int F(const P1 &p1, uint32_t i321, size_t *written) {                 \
    return ::N(WALLYP(p1), i321, written);                                     \
  }

#define WALLY_FN_P33_A(F, N)                                                   \
  template <class P1, class O>                                                 \
  inline int F(const P1 &p1, uint32_t i321, uint32_t i322, O **out) {          \
    return ::N(WALLYP(p1), i321, i322, out);                                   \
  }

#define WALLY_FN_P6B3(F, N)                                                    \
  template <class P1, class I1>                                                \
  inline int F(const P1 &p1, uint64_t i641, const I1 &i1, uint32_t i321) {     \
    return ::N(WALLYP(p1), i641, WALLYB(i1), i321);                            \
  }

#define WALLY_FN_P6BB33_B(F, N)                                                \
  template <class P1, class I1, class I2, class O>                             \
  inline int F(const P1 &p1, uint64_t i641, const I1 &i1, const I2 &i2,        \
               uint32_t i321, uint32_t i322, O &out) {                         \
    return ::N(WALLYP(p1), i641, WALLYB(i1), WALLYB(i2), i321, i322,           \
               WALLYO(out));                                                   \
  }

#define WALLY_FN_P6BBBBBB3(F, N)                                               \
  template <class P1, class I1, class I2, class I3, class I4, class I5,        \
            class I6>                                                          \
  inline int F(const P1 &p1, uint64_t i641, const I1 &i1, const I2 &i2,        \
               const I3 &i3, const I4 &i4, const I5 &i5, const I6 &i6,         \
               uint32_t i321) {                                                \
    return ::N(WALLYP(p1), i641, WALLYB(i1), WALLYB(i2), WALLYB(i3),           \
               WALLYB(i4), WALLYB(i5), WALLYB(i6), i321);                      \
  }

#define WALLY_FN_PB(F, N)                                                      \
  template <class P1, class I1> inline int F(const P1 &p1, const I1 &i1) {     \
    return ::N(WALLYP(p1), WALLYB(i1));                                        \
  }

#define WALLY_FN_PBBBBB(F, N)                                                  \
  template <class P1, class I1, class I2, class I3, class I4, class I5>        \
  inline int F(const P1 &p1, const I1 &i1, const I2 &i2, const I3 &i3,         \
               const I4 &i4, const I5 &i5) {                                   \
    return ::N(WALLYP(p1), WALLYB(i1), WALLYB(i2), WALLYB(i3), WALLYB(i4),     \
               WALLYB(i5));                                                    \
  }

#define WALLY_FN_PBBBBBB(F, N)                                                 \
  template <class P1, class I1, class I2, class I3, class I4, class I5,        \
            class I6>                                                          \
  inline int F(const P1 &p1, const I1 &i1, const I2 &i2, const I3 &i3,         \
...
#define WALLY_FN_PB33BP3(F, N)                                                 \
  template <class P1, class I1, class I2, class P2>                            \
  inline int F(const P1 &p1, const I1 &i1, uint32_t i321, uint32_t i322,       \
               const I2 &i2, const P2 &p2, uint32_t i323) {                    \
    return ::N(WALLYP(p1), WALLYB(i1), i321, i322, WALLYB(i2), WALLYP(p2),     \
               i323);                                                          \
  }

#define WALLY_FN_B33BPBBBBBB_A(F, N)                                           \
  template <class I1, class I2, class P1, class I3, class I4, class I5,        \
            class I6, class I7, class I8, class O>                             \
  inline int F(const I1 &i1, uint32_t i321, uint32_t i322, const I2 &i2,       \
               const P1 &p1, const I3 &i3, const I4 &i4, const I5 &i5,         \
               const I6 &i6, const I7 &i7, const I8 &i8, O **out) {            \
    return ::N(WALLYB(i1), i321, i322, WALLYB(i2), WALLYP(p1), WALLYB(i3),     \
               WALLYB(i4), WALLYB(i5), WALLYB(i6), WALLYB(i7), WALLYB(i8),     \
               out);                                                           \
  }

#define WALLY_FN_PB33BPBBBBBB3(F, N)                                           \
  template <class P1, class I1, class I2, class P2, class I3, class I4,        \
            class I5, class I6, class I7, class I8>                            \
  inline int F(const P1 &p1, const I1 &i1, uint32_t i321, uint32_t i322,       \
               const I2 &i2, const P2 &p2, const I3 &i3, const I4 &i4,         \
               const I5 &i5, const I6 &i6, const I7 &i7, const I8 &i8,         \
               uint32_t i323) {                                                \
    return ::N(WALLYP(p1), WALLYB(i1), i321, i322, WALLYB(i2), WALLYP(p2),     \
               WALLYB(i3), WALLYB(i4), WALLYB(i5), WALLYB(i6), WALLYB(i7),     \
               WALLYB(i8), i323);                                              \
  }

#define WALLY_FN_PB3_A(F, N)                                                   \
  template <class P1, class I1, class O>                                       \
  inline int F(const P1 &p1, const I1 &i1, uint32_t i321, O **out) {           \
    return ::N(WALLYP(p1), WALLYB(i1), i321, out);                             \
  }

#define WALLY_FN_PB3_B(F, N)                                                   \
  template <class P1, class I1, class O>                                       \
  inline int F(const P1 &p1, const I1 &i1, uint32_t i321, O &out) {            \
    return ::N(WALLYP(p1), WALLYB(i1), i321, WALLYO(out));                     \
  }

#define WALLY_FN_PB3_P(F, N)                                                   \
  template <class P1, class I1, class O>                                       \
  inline int F(const P1 &p1, const I1 &i1, uint32_t i321, O *out) {            \
    return ::N(WALLYP(p1), WALLYB(i1), i321, out);                             \
  }

#define WALLY_FN_PB_A(F, N)                                                    \
  template <class P1, class I1, class O>                                       \
  inline int F(const P1 &p1, const I1 &i1, O **out) {                          \
    return ::N(WALLYP(p1), WALLYB(i1), out);                                   \
  }

#define WALLY_FN_PP(F, N)                                                      \
  template <class P1, class P2> inline int F(const P1 &p1, const P2 &p2) {     \
    return ::N(WALLYP(p1), WALLYP(p2));                                        \
  }

#define WALLY_FN_PP3_BS(F, N)                                                  \
  template <class P1, class P2, class O>                                       \
  inline int F(const P1 &p1, const P2 &p2, uint32_t i321, O &out,              \
               size_t *written = 0) {                                          \
    size_t n;                                                                  \
    int ret = ::N(WALLYP(p1), WALLYP(p2), i321, WALLYO(out),                   \
                  written ? written : &n);                                     \
    return written || ret != WALLY_OK             ? ret                        \
           : n == static_cast<size_t>(out.size()) ? WALLY_OK                   \
                                                  : WALLY_EINVAL;              \
...
#define WALLY_FN_PP_BS(F, N)                                                   \
  template <class P1, class P2, class O>                                       \
  inline int F(const P1 &p1, const P2 &p2, O &out, size_t *written = 0) {      \
    size_t n;                                                                  \
    int ret =                                                                  \
        ::N(WALLYP(p1), WALLYP(p2), WALLYO(out), written ? written : &n);      \
    return written || ret != WALLY_OK             ? ret                        \
           : n == static_cast<size_t>(out.size()) ? WALLY_OK                   \
                                                  : WALLY_EINVAL;              \
  }

#define WALLY_FN_PS(F, N)                                                      \
  template <class P1> inline int F(const P1 &p1, size_t s1) {                  \
    return ::N(WALLYP(p1), s1);                                                \
  }

#define WALLY_FN_PSB(F, N)                                                     \
  template <class P1, class I1>                                                \
  inline int F(const P1 &p1, size_t s1, const I1 &i1) {                        \
    return ::N(WALLYP(p1), s1, WALLYB(i1));                                    \
  }

#define WALLY_FN_PSP(F, N)                                                     \
  template <class P1, class P2>                                                \
  inline int F(const P1 &p1, size_t s1, const P2 &p2) {                        \
    return ::N(WALLYP(p1), s1, WALLYP(p2));                                    \
  }

#define WALLY_FN_PS_A(F, N)                                                    \
  template <class P1, class O>                                                 \
  inline int F(const P1 &p1, size_t s1, O **out) {                             \
    return ::N(WALLYP(p1), s1, out);                                           \
  }

#define WALLY_FN_P_A(F, N)                                                     \
  template <class P1, class O> inline int F(const P1 &p1, O **out) {           \
    return ::N(WALLYP(p1), out);                                               \
  }

#define WALLY_FN_P_BS(F, N)                                                    \
  template <class P1, class O>                                                 \
  inline int F(const P1 &p1, O &out, size_t *written = 0) {                    \
    size_t n;                                                                  \
...
#define WALLY_FN_P_S(F, N)                                                     \
  template <class P1> inline int F(const P1 &p1, size_t *written) {            \
    return ::N(WALLYP(p1), written);                                           \
  }

#define WALLY_FN_P_P(F, N)                                                     \
  template <class P1, class O> inline int F(const P1 &p1, O *written) {        \
    return ::N(WALLYP(p1), written);                                           \
  }

#define WALLY_FN_S_S(F, N)                                                     \
  inline int F(size_t s1, size_t *written) { return ::N(s1, written); }
...
WALLY_FN_BBB3_BS(scriptsig_multisig_from_bytes,
...
WALLY_FN_PB3_A(bip32_key_from_parent_path_alloc,
...
inline int free_string(char *str) { return ::wally_free_string(str); }

inline int clear(void *p, size_t n) { return ::wally_bzero(p, n); }
...
WALLY_FN_P(tx_elements_input_issuance_free,
...
WALLY_FN_PBBBBB(tx_elements_output_commitment_set,
...
WALLY_FN_6BBBBBB_A(tx_elements_output_init_alloc,
                   wally_tx_elements_output_init_alloc)
WALLY_FN_PB33BPBBBBBB3(tx_add_elements_raw_input,
...
WALLY_FN_6_B(tx_confidential_value_from_satoshi,
             wally_tx_confidential_value_from_satoshi)
WALLY_FN_P6BB33_B(tx_get_elements_signature_hash,
                  wally_tx_get_elements_signature_hash)
WALLY_FN_B3B_B(tx_elements_issuance_generate_entropy,
               wally_tx_elements_issuance_generate_entropy)
WALLY_FN_B_B(tx_elements_issuance_calculate_asset,
             wally_tx_elements_issuance_calculate_asset)
WALLY_FN_B3_B(tx_elements_issuance_calculate_reissuance_token,
