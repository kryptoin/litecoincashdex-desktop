...
namespace atomic_dex
{
    ...
    kdf_service::~kdf_service()
    {
        ...
        if (!kdf_stopped)
        {
            ...
            // kill_executable() already handles platform differences internally
            // (pgrep/killall on macOS/Linux, taskkill on Windows), so call it
            // unconditionally.  Previously the non-Windows branch was commented
            // out, which left an orphaned KDF holding port 7783 after quit and
            // caused a "KDF did not initialize" failure on the next launch.
            ...
            try
            {
                ...
                if (answers.count("error") == 0)
                {
                    ...
                    for (auto&& coin: activated_coins)
                    {
                        ...
                        tickers.push_back(coin.ticker);
                        // LCC: defer balance fetch after activation; fetch_single_balance(coin);
                    }
        ...
        auto callback = [this]<typename RpcRequest>(RpcRequest rpc)
        {
            if (rpc.error)
            {
                ...
                if (rpc.error->error_type.find("PlatformIsAlreadyActivated") != std::string::npos)
                {
                    ...
                    lock.unlock();
                    dispatcher_.trigger(coin_fully_initialized{.tickers = {rpc.request.ticker}});
                    if constexpr (std::is_same_v<RpcRequest, kdf::enable_eth_with_tokens_rpc>)
                    {
                        ...
                        for (const auto& erc20_coin_info : rpc.request.erc20_tokens_requests)
                        {
                            ...
                            lock.unlock();
                            dispatcher_.trigger(coin_fully_initialized{.tickers = {erc20_coin_info.ticker}});
                        }
                    }
                }
                    ...
                    for (const auto& erc20_address_info : rpc.result->erc20_addresses_infos)
                    {
                        ...
                        else
                        {
                            for (const auto& balance : erc20_address_info.second.balances)
                            {
                                ...
                                dispatcher_.trigger(coin_fully_initialized{.tickers = {balance.first}});
        ...
        auto callback = [this]<typename RpcRequest>(RpcRequest rpc)
        {
                    ...
                    || rpc.error->error_type.find("TokenIsAlreadyActivated") != std::string::npos)
                {
                    ...
                    if constexpr (std::is_same_v<RpcRequest, kdf::enable_tendermint_with_assets_rpc>)
                    {
                        for (const auto& tendermint_coin_info : rpc.request.tokens_params)
                        {
                            ...
                            dispatcher_.trigger(coin_fully_initialized{.tickers = {tendermint_coin_info.ticker}});
                        }
                    }
                }
                else
            ...
            else
            {
                ...
                if constexpr (std::is_same_v<RpcRequest, kdf::enable_tendermint_with_assets_rpc>)
                {
                    ...
                    {
                        dispatcher_.trigger(coin_fully_initialized{.tickers = {tendermint_token_addresses_info.first}});
                ...
                if (rpc.error->error_type.find("PlatformIsAlreadyActivated") != std::string::npos || rpc.error->error_type.find("TokenIsAlreadyActivated") != std::string::npos)
                {
                    ...
                    if constexpr (std::is_same_v<RpcRequest, kdf::enable_bch_with_tokens_rpc>)
                    {
                        for (const auto& slp_coin_info : rpc.request.slp_tokens_requests)
                        {
                            ...
                            lock.unlock();
                            dispatcher_.trigger(coin_fully_initialized{.tickers = {slp_coin_info.ticker}});
                        }
                    }
                }
                else
                            ...
                            if (answers.count("error") == 0)
                            {
                                ...
                                for (auto&& answer: answers)
                                {
                                    ...
                                    if (!res)
                                    {
                                        ...
                                        if (error.find("CoinIsAlreadyActivated") != std::string::npos)
                                        {
                                            ...
                                            this->dispatcher_.trigger(coin_fully_initialized{.tickers = {tickers[idx]}});
                                            ...
                                            auto task_id = answer.at("result").at("task_id").get<std::int8_t>();
                                            {
                                                ...
                                                do {
                                                    ...
                                                    if (status == "Ok")
                                                    {
                                                        ...
                                                        dispatcher_.trigger(coin_fully_initialized{.tickers = {tickers[idx]}});
                                                        break;
                                                    }
                                ...
                                if (!tickers.empty())
                                {
                                    ...
                                    if (tickers.size() == 1)
                                    {
                                        ...
                                        // LCC: defer balance fetch after activation; fetch_single_balance(get_coin_info(tickers[0]));
                                    }
                                }
                            }
                        }
    ...
    void kdf_service::spawn_kdf_instance(std::string wallet_name, std::string passphrase, bool with_pin_cfg, std::string rpcpass)
    {
        ...
        m_kdf_init_thread = std::thread(
            [this, kdf_cfg_path]()
            {
                ...
                // start (binary not cached, first launch after reboot) this can
                // easily take longer than 30s, which previously triggered a
                // false "did not initialize" fatal. Allow up to 3 minutes and
                // log progress so slow startups are visible instead of silent.
                static constexpr std::size_t kdf_max_tries = 180;
                ...
                while (not check_kdf_alive())
                {
                    ...
                    if (nb_try % 30 == 0)
                    {
                        SPDLOG_WARN("KDF not responsive yet, waited {}s (max {}s)", nb_try, kdf_max_tries);
                    }
                    if (nb_try >= kdf_max_tries)
                    {
                        SPDLOG_ERROR("KDF not started correctly after {} seconds", kdf_max_tries);
                        dispatcher_.trigger(fatal_notification{
                            "KDF (mm2_cheetah) daemon did not become responsive after 3 minutes.\n\n"
